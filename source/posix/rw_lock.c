...
static const ssize_t s_max_readers = INT32_MAX;
...
int aws_rw_lock_rlock(struct aws_rw_lock *lock) {
    ...
    const ssize_t num_readers = aws_atomic_fetch_add(&lock->readers, 1) + 1;
    if (num_readers < 0) {
...
int aws_rw_lock_wlock(struct aws_rw_lock *lock) {
    ...
    const ssize_t num_readers = aws_atomic_fetch_sub(&lock->readers, s_max_readers);
    if (num_readers > 0) {
        const ssize_t num_holdouts = aws_atomic_fetch_add(&lock->holdouts, num_readers) + num_readers;
...
int aws_rw_lock_runlock(struct aws_rw_lock *lock) {
    ...
    const ssize_t num_readers = aws_atomic_fetch_sub(&lock->readers, 1) - 1;

    if (num_readers < 0) {
        const ssize_t num_holdouts = aws_atomic_fetch_sub(&lock->holdouts, 1) - 1;
        if (num_holdouts == 0) {
...
int aws_rw_lock_wunlock(struct aws_rw_lock *lock) {
    ...
    const ssize_t current = aws_atomic_fetch_add(&lock->readers, s_max_readers) + s_max_readers;
    assert(current >= 0);
